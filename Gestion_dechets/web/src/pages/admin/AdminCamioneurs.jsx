...
export default function AdminCamioneurs() {
  ...
  const [form, setForm] = useState({
    email: '', nom: '', password: 'ChangeMe123!', numero_permis: '', langue: 'FR',
  });
  const debouncedSearch = useDebouncedValue(search);
  const toast = useToast();

  const fetchCamioneurs = useCallback(() => {
    setLoading(true);
    const params = {};
    if (debouncedSearch) params.search = debouncedSearch;
    if (filter) params.statut = filter;
    api.get('/api/admin/camioneurs/', { params })
      .then((res) => setCamioneurs(unwrapList(res.data)))
      .catch(() => toast('Erreur lors du chargement.', 'error'))
      .finally(() => setLoading(false));
  }, [debouncedSearch, filter, toast]);

  useEffect(() => { fetchCamioneurs(); }, [fetchCamioneurs]);

  const handleAction = async (id, action) => {
    try {
      await api.post(`/api/admin/camioneurs/${id}/valider/`, { action });
      toast(action === 'accepter' ? 'Camioneur validé !' : 'Compte refusé.', action === 'accepter' ? 'success' : 'error');
      fetchCamioneurs();
    } catch (err) {
  ...
  const handleCreate = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post('/api/admin/camioneurs/', { ...form, role: 'camioneur' });
      toast('Camioneur créé. Mot de passe temporaire à changer à la 1ère connexion.', 'success');
      setShowModal(false);
      setForm({ email: '', nom: '', password: 'ChangeMe123!', numero_permis: '', langue: 'FR' });
      fetchCamioneurs();
    } catch (err) {
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">Gestion des Camioneurs</h1>
          <p className="page-subtitle">Validation et supervision des comptes camioneurs</p>
        </div>
        <button type="button" className="btn btn-primary" onClick={() => setShowModal(true)}>
          <MdAdd /> Nouveau camioneur
        </button>
      </div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6}><div className="empty-state">Aucun camioneur trouvé.</div></td></tr>
              ) : camioneurs.map((c) => {
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td>{c.email}</td>
                    <td><code>{c.numero_permis || '—'}</code></td>
                    <td>
                      <span className={`badge ${c.disponible ? 'badge-success' : 'badge-neutral'}`}>
                        {c.disponible ? 'Oui' : 'Non'}
                      </span>
                    </td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="modal-title">Créer un camioneur</h2>
            ...
            <form className="modal-form" onSubmit={handleCreate}>
              ...
              <div className="form-group">
                ...
                <input className="form-input" value={form.numero_permis} onChange={(e) => setForm({ ...form, numero_permis: e.target.value })} required />
              </div>
              <div className="form-group">
